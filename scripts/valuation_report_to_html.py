#!/usr/bin/env python3
"""Convert valuation markdown report to dedicated HTML page."""
...
details{margin:16px 0}summary{cursor:pointer;font-size:13px;font-weight:500;text-transform:uppercase;letter-spacing:1px;color:var(--text3);padding:10px 0;border-bottom:1px solid var(--border)}details .details-content{padding:16px 0;font-size:14px;color:var(--text2)}
...
def extract_value_from_table(md_text: str, key: str) -> str:
...
def extract_header(md_text: str) -> dict:
    title = re.search(r"#\s+估值分析报告：(.+?)（(.+?)）", md_text)
    date = re.search(r"分析日期[:：]\s*(\d{4}-\d{2}-\d{2})", md_text)
    return {
...
def build_summary(md_text: str) -> dict:
    current_price = extract_value_from_table(md_text, "当前股价")
    intrinsic = extract_value_from_table(md_text, "定性调整后综合内在价值")
    judgment = extract_value_from_table(md_text, "**估值判断**") or extract_value_from_table(md_text, "估值判断")
    cv_match = re.search(r"CV\s*=\s*([\d.]+%)", md_text)
    consistency_match = re.search(r"方法一致性\s*\|\s*(.+?)\s*\|", md_text)
    return {
        "current_price": current_price,
        "intrinsic": intrinsic,
        "judgment": judgment.replace("**", "") if judgment else "",
        "cv": cv_match.group(1) if cv_match else "",
        "consistency": consistency_match.group(1).replace("**", "") if consistency_match else "",
    }


def verdict_style(judgment: str) -> tuple[str, str, str]:
    if "低估" in judgment:
        return "v-green", "tag-green", judgment
    if "偏高" in judgment or "高估" in judgment:
        return "v-red", "tag-red", judgment
    return "v-amber", "tag-amber", judgment or "N/A"


def main() -> None:
    parser = argparse.ArgumentParser(description="Convert valuation report MD to HTML page")
    parser.add_argument("--input", required=True, help="Path to valuation markdown report")
    parser.add_argument("--output", required=True, help="Output HTML path")
    parser.add_argument("--template", default=None, help="Jinja2 template path")
    parser.add_argument("--standalone", action="store_true", help="Embed CSS inline for local viewing")
    args = parser.parse_args()

    project_root = Path(__file__).resolve().parent.parent
    input_path = Path(args.input)
    output_path = Path(args.output)
    template_path = Path(args.template) if args.template else project_root / "strategies" / "valuation" / "references" / "valuation_report_dashboard.html"
    ...
    verdict_class, verdict_tag_class, verdict_tag = verdict_style(summary["judgment"])

    overview_html = md_to_html(extract_section(md_text, "报告概览"))
    executive_html = md_to_html(extract_section(md_text, "Executive Summary"))
    methods_html = md_to_html(extract_section(md_text, "四、估值方法详情（引用 valuation_computed.md + 定性调整）"))
    xval_html = md_to_html(extract_section(md_text, "五、交叉验证"))
    reverse_html = md_to_html(extract_section(md_text, "六、反向估值：当前价格隐含了什么？"))
    conclusion_html = md_to_html(extract_section(md_text, "七、估值结论"))
    risks_html = md_to_html(extract_section(md_text, "八、关键假设与风险提示"))
    wacc_html = md_to_html(extract_section(md_text, "二、WACC 计算（引用 valuation_computed.md）"))
    classification_html = md_to_html(extract_section(md_text, "一、公司分类"))

    key_cards = [
        {"label": "当前股价", "value": summary["current_price"], "css_class": "", "sub": ""},
        {"label": "综合内在价值", "value": summary["intrinsic"], "css_class": "highlight" if "低估" in summary["judgment"] else ("warn" if "偏高" in summary["judgment"] or "高估" in summary["judgment"] else "amber-hl"), "sub": ""},
        {"label": "估值判断", "value": summary["judgment"], "css_class": "warn" if "偏高" in summary["judgment"] or "高估" in summary["judgment"] else ("highlight" if "低估" in summary["judgment"] else "amber-hl"), "sub": ""},
        {"label": "方法一致性", "value": summary["consistency"], "css_class": "", "sub": f"CV = {summary['cv']}" if summary["cv"] else ""},
    ]
    ...
    html = Environment(loader=BaseLoader()).from_string(template_text).render(
        ...
        verdict_text=f"当前股价 {summary['current_price']}，综合内在价值 {summary['intrinsic']}，估值判断 {summary['judgment']}" if summary["current_price"] and summary["intrinsic"] else summary["judgment"],
        key_cards=key_cards,
    ...
    print(f"  Has overview: {bool(overview_html)}")
    print(f"  Has methods: {bool(methods_html)}")
    print(f"  Has cross validation: {bool(xval_html)}")
    print(f"  Has reverse valuation: {bool(reverse_html)}")
    print(f"  Has risks: {bool(risks_html)}")
