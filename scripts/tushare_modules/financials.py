...
class FinancialsMixin:
    ...
    def _newest_first_as_of(self, df: pd.DataFrame, date_column: str) -> pd.DataFrame:
        """Return eligible rows newest-first without trusting API row order."""
        if df.empty or date_column not in df.columns:
            return df.copy()
        ordered = df.copy()
        ordered["_normalized_sort_date"] = ordered[date_column].map(self._compact_date)
        ordered = ordered[
            ordered["_normalized_sort_date"].notna()
            & (ordered["_normalized_sort_date"] <= self.as_of)
        ]
        return ordered.sort_values(
            "_normalized_sort_date", ascending=False
        ).drop(columns="_normalized_sort_date").reset_index(drop=True)
    ...
    def get_basic_info(self, ts_code: str) -> str:
        ...
        if not daily.empty:
            daily = self._newest_first_as_of(daily, "trade_date")
        val_rows = []
        if not daily.empty:
    ...
    def _get_basic_info_hk(self, ts_code: str) -> str:
        ...
        val_rows = []
        try:
            fina = self._cached_call("hk_fina_indicator", ts_code=ts_code,
                                   fields="ts_code,end_date,pe_ttm,pb_ttm,total_market_cap,hksk_market_cap")
            if not fina.empty:
                fina = self._newest_first_as_of(fina, "end_date")
            if not fina.empty:
    ...
    def get_market_data(self, ts_code: str) -> str:
        ...
        df = self._newest_first_as_of(df, "trade_date")
        if df.empty:
            lines.append("数据缺失\n")
            return "\n".join(lines)
