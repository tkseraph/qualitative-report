...
def test_html_validator_requires_mobile_chart_density_and_scroll_region():
    markdown = "\n".join(f"## 维度{x}" for x in "一二三四五六") + """
...
<details data-component-role="moat-interrogation" open></details>
<details data-component-role="moat-falsification" open></details>
<div class="chart-container" data-chart-id="mobile-check" data-chart-series='{"datasets":[{"label":"收入","unit":"亿元","role":"bar"},{"label":"ROE","unit":"%","role":"line"}]}'></div>
</body></html>"""

    errors, manifest = validate_html(html, markdown)

    assert any("reduced-density mobile rendering" in error for error in errors)
    assert manifest["validation"]["mobile_chart_contract"] is False


def test_html_validator_rejects_stale_mobile_render_contract():
    markdown = "\n".join(f"## 维度{x}" for x in "一二三四五六")
    html = """<html><head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="qualitative-mobile-contract" content="1.0">
</head><body>
<h2>维度一</h2><h2>维度二</h2><h2>维度三</h2>
<h2>维度四</h2><h2>维度五</h2><h2>维度六</h2>
<details data-component-role="moat-interrogation" open></details>
